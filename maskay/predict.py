...
def predict_tensor(
    tensor: torch.Tensor, device: str = "cuda", model: str = "adan"
) -> torch.Tensor:
...
def predict_SAFE(
    ...
    xinit, yinit, dim10, dim20, dim60, s2crs = obtain_img_metadata(
        allS2bands10=allS2bands10, allS2bands20=allS2bands20, allS2bands60=allS2bands60
    )
    ...
    batch_size = fix_batch_size(
        batch_size=batch_size, dim=dim, mindimreq=SegModel.maskayparams.mindimreq
    )
...
def retrieve_model(model: str = "adan"):
    ...
    model = model.lower()
    available_models = [
        _model for _model in _models_available if re.search(model, _model)
    ]

    if len(available_models) == 0:
        raise ValueError(
            f"Model {model} is not supported. Available models: {_models_available}"
        )
...
if __name__ == "__main__":
    ...
    a = time.time()
    predict_SAFE(
        folder, output, device="cpu", model="adan", batch_size=10000, res=34.3125
    )
    time.time() - a
